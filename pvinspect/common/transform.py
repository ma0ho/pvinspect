...
def _brown(coords, coeffs):
    ...
    factor = 1 + np.expand_dims(np.sum([kn*(radius**(2*(n+1))) for n, kn in enumerate(coeffs)], axis = 0), axis = -1)
...
def _brown_oder1_inverse(coords, k):
    ...
    if np.any(g1):
        ...
        rd[g1] = np.cbrt(1/k * (ru[g1]/2 + tmp)) + np.cbrt(1/k * (ru[g1]/2 - tmp))
    g2 = D < 0
    if np.any(g2):
        rd[g2] = 2*np.sqrt(-3*k)/(3*k) * np.cos(1/3 * np.arccos(27*k**2*ru[g2] / (2*np.sqrt(-(3*k)**3))) + 4*np.pi/3)
...
def distort_inverse(coords, params):
    if len(params) > 1:
        raise NotImplementedError('Inversion not implemented for more than 1 coefficient')
...
def _normalize_transform(coords, new_centroid = (0,0), new_mean_distance = np.sqrt(2)):
    ...
    scaling = new_mean_distance/np.mean(np.linalg.norm(coords-centroid.reshape(1,-1), axis = 1))
    return np.array([[scaling, 0, -centroid[0]],
                     [0, scaling, -centroid[1]],
                     [0, 0, 1]])
...
def _calibrate_camera_wrap(src, dest, image_width, image_height, n_dist_coeff, mask = None):
    # coords in OpenCV compatible format
    src2 = []
    for src_i in src:
        src_i = np.concatenate([src_i, np.zeros((src_i.shape[0], 1))], axis = 1)
...
def find_full_model(src, dest, n_dist_coeff, image_width, image_height, ransac = False):
    ...
    errs = [np.mean(np.linalg.norm(src_i-apply_homography(dest_i, np.linalg.pinv(H_i)), axis = 1)) for H_i, dest_i, src_i in zip(H, dest, src)]
    ...
    if ransac:
        ...
        def err_f(model):
            ...
            dest_t = [apply_pinhole(dest_i, Rt_i, A, [0], inverse = True) for dest_i, Rt_i in zip(dest, Rt)]
            err = [np.mean(np.linalg.norm(src_i-dest_t_i, axis = 1)) for src_i, dest_t_i in zip(src, dest_t)]
    ...
    A, Rt, k = _calibrate_camera_wrap(src, dest, image_width, image_height, n_dist_coeff)
...
def warp_image(img, transform, x0, y0, xstep, ystep, w, h, method = 'linear'):
    ...
    v = scipy.interpolate.interpn(
        (np.arange(img.shape[0], dtype = np.float32), np.arange(img.shape[1], dtype = np.float32)),
        img,
        np.fliplr(samples),
...
class Transform(ABC):
    ...
    def mean_scale(self):
        ...
        at = self(a)
        s1 = 1/2 * (np.linalg.norm(at[1]-at[0]) + np.linalg.norm(at[3]-at[2])) / 10
        s2 = 1/2 * (np.linalg.norm(at[2]-at[1]) + np.linalg.norm(at[0]-at[3])) / 10
        s = 1/2 * (s1+s2)
...
class HomographyTransform(Transform):
    ...
    def __init__(self, src, dest, image_width = 0, image_height = 0, ransac = False, ransac_thres = 0.1):
        ...
        self._err = np.mean(np.linalg.norm(apply_homography(src, self._H) - dest, axis = 1))
...
class FullTransform(Transform):
    ...
    def __init__(self, src, dest, image_width = 0, image_height = 0, ransac = False, n_dist_coeff = 1):
        Rt, self._A, self._dist = find_full_model([src], [dest], n_dist_coeff, image_width, image_height, ransac = ransac)
        self._Rt = Rt[0]
    ...
    def __call__(self, coords):
        if self._success:
            return apply_pinhole(coords, self._Rt, self._A, self._dist, self._is_inverse)
        else:
            return None
...
class FullMultiTransform(MultiTransform):
    ...
    def __init__(self, src, dest, image_width = 0, image_height = 0, ransac = False, n_dist_coeff = 1, fixed_intrinsics = None):
        if ransac:
            assert fixed_intrinsics is None

        if ransac:
            self._Rt, self._A, self._dist,  self._mask = find_full_model(src, dest, n_dist_coeff, image_width, image_height, ransac = ransac)
        else:
            if fixed_intrinsics is None:
                self._Rt, self._A, self._dist = find_full_model(src, dest, n_dist_coeff, image_width, image_height, ransac = ransac)
            else:
    ...
    def __call__(self, coords):
        if self._success:
            return [apply_pinhole(x, rt, self._A, self._dist, self._is_inverse) for x, rt in zip(coords, self._Rt)]
        else:
            return None
    ...
    def get_parameter_handles(self, items, extrinsic = True, intrinsic = False, distortion = False):
        res = []
        res.append(self[items[0]].get_parameter_handles(extrinsic = extrinsic, intrinsic = intrinsic, distortion = distortion))

        if extrinsic:
            for i in items[1:]:
                res.append(self[i].get_parameter_handles(extrinsic = extrinsic, intrinsic = False, distortion = False))
    ...
    def update_parameters_by_handles(self, handles, items, extrinsic = True, intrinsic = False, distortion = False):
        ...
        l = t[0].get_parameter_handles(extrinsic = extrinsic, intrinsic = intrinsic, distortion = distortion).shape[0]
        t[0].update_parameters_by_handles(handles[:l], extrinsic = extrinsic, intrinsic = intrinsic, distortion = distortion)
