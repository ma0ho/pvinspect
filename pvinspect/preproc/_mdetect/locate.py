...
from .config import CORNER_DETECTION_PATCH_SIZE, RANSAC_THRES, PREPROC_LOWER_PERCENTILE, PREPROC_UPPER_PERCENTILE
import numpy as np
from typing import Tuple
from skimage import exposure
from scipy.ndimage.filters import median_filter
    
def apply(img: np.ndarray, n_cols: int, m_rows: int, is_module_detail: bool, orientation: str = None) -> Tuple[transform.Transform, np.ndarray, np.ndarray, np.ndarray]:
    ...
    module_bb = locate_module.module_boundingbox_model(module_bb_fit, n_cols, m_rows, orientation)
    ...
    outer_corners_fit, outer_accepted = locate_corners.fit_outer_corners(img, outer_corners, locate_transform, CORNER_DETECTION_PATCH_SIZE, n_cols, m_rows, is_module_detail=is_module_detail)
    outer_corners_fit = locate_transform(outer_corners_fit)

    # reestimate transform
    locate_transform = transform.HomographyTransform(outer_corners[outer_accepted], outer_corners_fit[outer_accepted], ransac=True)
