...
def resize(
    ...
    scale_factors, out_shape = set_scale_and_out_sz(
        in_shape, out_shape, scale_factors, fw
    )
    ...
    def forward(self, input):
        ...
        for (dim, scale_factor), field_of_view, weights in zip(
            ...
            output = apply_weights(
                output, field_of_view, weights, dim, self.n_dims, torch
            )
        return output
...
def prepare_weights_and_field_of_view_1d(
    ...
    # that influence it
    field_of_view = get_field_of_view(projected_grid, cur_support_sz, in_sz, fw, eps)

    # STEP 3- CALCULATE WEIGHTS: Match a set of weights to the pixels in the
    # field of view for each output pixel
    weights = get_weights(interp_method, projected_grid, field_of_view.float())
...
def get_projected_grid(in_sz, out_sz, scale_factor, fw, device=None):
    ...
    out_coordinates = fw.arange(out_sz).float()
    ...
    return (
        out_coordinates / scale_factor
        + (in_sz - 1) / 2
        - (out_sz - 1) / (2 * scale_factor)
    )
