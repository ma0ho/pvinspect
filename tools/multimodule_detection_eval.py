...
def objective(trial: optuna.Trial, imgs, labels) -> float:
    ...
    _, boxes = locate_multiple_modules(
        ...
        filter_size=trial.suggest_discrete_uniform("filter_size", 0, 30, 2)
        if isinstance(trial, optuna.Trial)
        else trial.params["filter_size"],
    ...
    for img in imgs:
        ...
        if img.path in boxes.keys():
            pred = boxes[img.path]
            iou_per_image[img.path.name] = mean_iou(
                [polygon2boundingbox(x[1]) for x in label], pred
            )
        else:
